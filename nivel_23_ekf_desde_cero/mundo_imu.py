"""El mundo del acto 4: DUPLICADO del nivel 22, a propósito (regla 2).

Misma semilla, mismo circuito, misma IMU con sesgo, mismo apagón: las
MEDIDAS son idénticas bit a bit a las que vio el grafo VIO del nivel 22.
Así, cuando el acto 4 compare filtro contra smoother, la única diferencia
será el estimador — no el mundo.

Un vehículo 2D "tipo dron": SIN odometría. Sus dos sentidos son la VISIÓN
(landmarks, solo en los keyframes, a ~4 Hz) y una IMU (giróscopo +
acelerómetro, a 100 Hz, con ruido y con SESGO). Y en el tramo más incómodo
del circuito — una CURVA — la visión se apaga 5 segundos.
...
muestras (4 Hz). El ground truth existe a ambas frecuencias; el filtro
solo ve medidas.
──────────────────────────────────────────────────────────────────────────────
"""
...
BIAS_GYRO_REAL = 0.03      # rad/s — el sesgo que el filtro debera DESCUBRIR
...
def generar(semilla: int = 11, con_apagon: bool = True,
    ...
    vuelta — unos 5 s que incluyen el giro completo.
    """
