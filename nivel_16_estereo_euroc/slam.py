"""El SLAM métrico del nivel 15, alimentado por ESTÉREO real.

El cambio honesto de este nivel: (casi) NINGUNO. En el nivel 15 la cámara
derecha era VIRTUAL — u_R = u − bf/z se sintetizaba desde la profundidad del
Kinect, con bf = 40 por convención. Aquí la cámara derecha es REAL: u_R se
MIDE (u_R = u_L − d, el match en la imagen derecha, hecho denso por SGBM en
dataset.py), y bf lo entrega la CALIBRACIÓN del rig (bf = −P2[0,3], ~47.9 en
EuRoC). Mismo residuo [u, v, u_R], misma init instantánea, mismo bucle SE(3).
Solo cambia de dónde sale z. Esa es la lección 37 del padre: un backend bien
factorizado no distingue el sensor.

Dos diferencias visibles en el código: `bf` entra por el constructor (ya no
es la constante STEREO_BF, porque ahora es una MEDICIÓN del rig), y entra la
RELOCALIZACIÓN — la pieza que veníamos aplazando desde el nivel 13 y que el
dron EXIGE: sin ella, un solo tropiezo en el vuelo 6-DoF cuesta el resto de
la secuencia (lo medimos: 1141 frames y 54 cm). Ver las constantes RELOC_*.

─── La matemática que sigue mandando (nivel 15, íntegra) ─────────────────────
La escala es MEDICIÓN, no gauge: z = bf/d viene de la geometría del par. El
ATE se evalúa con alineación RÍGIDA y la escala de similitud se calcula
APARTE (≈ 1.000 = metros de verdad). El bucle va en SE(3): un bucle Sim(3)
re-escalaría un mapa cuyos puntos siguen naciendo métricos — composición de
error (lección 35). Y el peso del residuo u_R decae con z² justo cuando el
ruido de la profundidad estéreo crece con z² (∂z/∂d = −bf/d²): la geometría
compensa el ruido en la dirección correcta.
──────────────────────────────────────────────────────────────────────────────
"""
...
class SLAM:
    """INIT / TRACK / LOST + mapa + BA + bucle — todo en METROS.

    Identico al nivel 15: le da igual que la z venga de un Kinect o de la
    disparidad de un rig (dataset.py entrega la misma firma (ts, gris, z)).
    """
    ...
    # ── relocalizacion (lo que el dron EXIGE — nueva en este nivel) ──
    # En los niveles 14/15 perderse era casi siempre terminal pero raro
    # (secuencias de mano, movimiento moderado). Un dron en 6-DoF es otra
    # cosa: el coast por velocidad constante EXTRAPOLA un vuelo agresivo y
    # diverge en metros en segundos — sin una salida, un solo tropiezo
    # cuesta el resto de la secuencia (medido aqui: perderse en el frame
    # ~1770 costo 1141 frames y 54 cm). La reloc es la salida: reconocer
    # DONDE estamos contra toda la base de keyframes y saltar alli — con
    # la misma vara geometrica que el bucle: PnP con >= 40 inliers, o nada.
    RELOC_TRAS_LOST = 5        # intentar tras N frames perdidos seguidos
    RELOC_MIN_INLIERS = 40

    # ── la profundidad por disparidad (lo nuevo del nivel 16) ──
    DEPTH_MIN = 0.3            # mas alla de ~8 m la disparidad es sub-pixel
    DEPTH_MAX = 8.0            # y el ruido ~z² domina: mejor no crear mapa ahi
    DEPTH_MAX_NEW_POINTS = 400  # tope de puntos nuevos por KF desde profundidad
    # (bf ya NO es una constante de clase: en estereo real lo mide la
    #  calibracion del rig y entra por el constructor.)

    def __init__(self, K: np.ndarray, bf: float, usar_ba: bool = True,
        ...
        # bf = fx·b MEDIDO por la calibracion del rig (dataset.py). La
        # ablacion bf = 0 apaga el residuo estereo en el BA.
        self.residuo_bf = float(bf) if usar_residuo else 0.0
    ...
    def _kfs_locales(self) -> set:
        ...
        # Tras una RELOCALIZACION, re-anclar el mapa local en el keyframe
        # reconocido y sus covisibles: la recencia apunta a la zona de ANTES
        # de perderse y el tracking no podria continuar (el padre descubrio
        # este gap al medir su matching guiado — su leccion 24). Se limpia al
        # insertar el proximo keyframe (la recencia vuelve a valer).
    ...
    def _coast(self, info, kps=None, desc=None) -> None:
        ...
        RELOCALIZACION (teoria en las constantes RELOC_*)."""
    ...
    def _relocalizar(self, kps, desc, info) -> None:
        """El mismo pipeline del bucle, SIN filtro temporal: reconocer lugar
        (descriptores fuertes) -> verificar con geometria (PnP contra los
        puntos 3D del candidato) -> saltar a esa pose. La verificacion es lo
        que impide re-engancharse a una pose equivocada — que seria peor que
        seguir perdido (el aviso del ejercicio 4 del nivel 15)."""
        desc_pr = self._desc_fuertes(kps, desc)
        ...
        if mejor is None or mejor_n < self.LOOP_MIN_MATCHES:
            return
        ms = self._match(desc, mejor["desc"])
        ...
        self.T_rel = np.eye(4)            # el prior de velocidad ya no vale
        self.estado = "TRACK"
        self._lost_seguidos = 0
        self._ref_local_kf = mejor["id"]  # re-anclar el mapa local AQUI
    ...
    def _insertar_kf(self, kps, desc, n_inliers, info, pares_mapa) -> None:
        ...
        if self._depth is not None:
            ...
            if cand:
                ...
                # FILTRO ANTI-DUPLICADOS (del padre, v0.4 — y otra carencia
                # que este dron desnudo): un arranque ESTATICO inserta
                # keyframes desde la misma pose, y ORB no re-detecta identico
                # -> sin filtro, cada keyframe re-crea cientos de COPIAS de
                # los mismos puntos y el PnP salta entre copias. Lo medimos:
                # el arranque quieto de V1_01 vagaba ~1 cm/frame (p95 1.5 cm
                # ¡sin moverse!) y desplazo el ancla de la sesion ~1.5 m. Si
                # el candidato cae a < 1.5% de su profundidad de un punto
                # local existente, es la misma caracteristica fisica: se
                # DESCARTA la creacion (descartar no inventa observaciones
                # falsas; fusionar por proyeccion si podria).
                pids_loc = self.mapa.puntos_de_kfs(self._kfs_locales())
                valida = np.ones(len(pts_w), dtype=bool)
                if pids_loc:
                    pts_loc = np.array([self.mapa.puntos[p]
                                        for p in pids_loc])
                    for n in range(len(pts_w)):
                        d_min = float(np.min(np.linalg.norm(
                            pts_loc - pts_w[n], axis=1)))
                        if d_min < 0.015 * z[n]:
                            valida[n] = False
    ...
    def procesar(self, gray: np.ndarray,
        ...
        """Un frame métrico: imagen gris + mapa de profundidad en METROS
