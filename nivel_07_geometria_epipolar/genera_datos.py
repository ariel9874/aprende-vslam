...
(scripts/make_synthetic_sequence.py, modo forward), con los planos
REPOSICIONADOS para este nivel: en la escena del nivel 08, la primera vista
la domina un solo plano (91% de los matches) y la estimación de E DEGENERA
(una escena plana admite una familia de E's — la lección 2 del repo padre;
aquí medimos errT de 80 grados con la E ajustando a 0.4 px). Esta variante
reparte los matches entre las tres profundidades (~10/53/35) y la pose sale
limpia (<0.5 grados). El EJERCICIO 5 del nivel te hace redescubrir la
degeneración a propósito.
...
def main() -> int:
    ...
    # Tres planos a profundidades distintas y REPARTIDOS por el campo visual
    # de la primera vista (medido: ~10/53/35% de los matches por plano). Si
    # un solo plano domina, E degenera — ver el docstring y el ejercicio 5.
    planes = [  # de lejano a cercano (el orden resuelve la oclusión)
        TexturedPlane(2.0, 0.0, 14.0, 12.0, 7.0, make_texture(1800, 1000, rng)),
        TexturedPlane(-1.5, 0.9, 7.0, 2.4, 1.9, make_texture(700, 550, rng)),
        TexturedPlane(1.8, -0.9, 4.0, 1.5, 1.1, make_texture(560, 420, rng)),
    ]
