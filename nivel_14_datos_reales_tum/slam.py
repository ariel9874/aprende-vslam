"""El SLAM del nivel 13, endurecido para DATOS REALES.

Mismo esqueleto (INIT / TRACK / LOST, mapa por keyframes, BA de ventana,
cierre de bucle verificado) más las CUATRO piezas que cruzan el salto
sim->real, cada una una lección medida del repo padre:

  1. MATCHING GUIADO por reproyección (su lección 24): la palanca de robustez.
  2. MAPA LOCAL con COVISIBILIDAD y keyframes que OBSERVAN antes de crear
     (lección 14): sin esto, una sesión larga fabrica un mapa de duplicados
     que ningún BA puede arreglar — lo medimos: 96 000 puntos y 35 cm.
  3. OBSERVACIONES PUENTE al cerrar bucle (lección 26): lo que ata la escala.
  4. BUNDLE ADJUSTMENT GLOBAL offline (lecciones 26-27): el refinador final.

─── La matemática: por qué el matching guiado gana ───────────────────────────
El matching global por descriptor ignora la geometría: contra un mapa real
(miles de puntos ORB) produce muchas asociaciones ambiguas — el repo padre lo
midió: ~100-150 matches con CERO inliers geométricos (su lección 22). La cura:
PREDECIR la pose (velocidad constante), PROYECTAR el mapa local a la imagen, y
buscar cada punto sólo entre los keypoints a < 15 px de su proyección. Dentro
de esa ventana el vecino correcto casi no tiene rival: suben los inliers
verdaderos Y baja el ruido. Es el "track local map" de ORB-SLAM. Medido por el
padre (un solo cambio): fr2_desk 104.9 cm/1347 perdidos -> 21.9 cm/0.

─── La matemática: por qué un BA global OFFLINE ──────────────────────────────
El grafo Sim(3) del bucle corrige POSES, pero no re-estima los PUNTOS: la
escala intermedia queda mal. El BA global (todos los keyframes + puntos, 2
anclas) sí — y las OBSERVACIONES PUENTE que registra el cierre de bucle (el
keyframe actual ve puntos del segmento viejo) atan los extremos de la cadena.
¿Por qué offline y no tras cada bucle? El padre lo probó ONLINE y descarrila:
el BA sacude el mapa en caliente y el tracking se pierde (fr2_xyz: 5 -> 346
frames perdidos). Como la métrica es la trayectoria FINAL de keyframes
(lección 25), UN BA al terminar da el beneficio sin tocar el tracking. Es el
"full BA" offline de ORB-SLAM. Medido: fr2_xyz 13 -> 0.4 cm... con 50
iteraciones. Con 10 se quedaba a mitad de camino (lección 27: antes de creer
que un residual es "de fondo", verifica que el optimizador CONVERGIÓ).
──────────────────────────────────────────────────────────────────────────────
"""
...
class SLAM:
    ...
    def __init__(self, K: np.ndarray, usar_ba: bool = True,
                 usar_bucle: bool = True, usar_guiado: bool = True) -> None:
        self.K = K
