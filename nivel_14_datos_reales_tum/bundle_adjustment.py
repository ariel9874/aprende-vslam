...
def residual_and_jacobians(K, T_c_w, X_w, uv):
    """Residuo y jacobianos de UNA observación (teoría en la cabecera).
    ...
    r = np.array([fx * x / z + cx - uv[0], fy * y / z + cy - uv[1]])
    d_pi = np.array([[fx / z, 0.0, -fx * x / z ** 2],
                     [0.0, fy / z, -fy * y / z ** 2]])
...
def bundle_adjustment(
    ...
    for _ in range(iterations):
        ...
        for k, p, uv in obs:
            out = residual_and_jacobians(K, T_c_w[k], pts[p], uv)
            if out is None:
                continue
